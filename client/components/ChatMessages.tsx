...
const UrlPreview = ({ linkPreview, url }: { linkPreview: any, url: string }) => {
  ...
  return (
    <a href={url} target="_blank" rel="noopener noreferrer" className="block mt-1 mb-2 w-full rounded-xl overflow-hidden border border-outline-variant/30 bg-surface-container-low hover:opacity-95 transition-all shadow-sm select-none">
      {data.image?.url && (
      ...
      <div className="p-3">
        {data.title && <p className="text-on-surface font-bold text-sm line-clamp-2 leading-snug">{data.title}</p>}
        {data.description && <p className="text-on-surface-variant text-xs line-clamp-2 mt-1">{data.description}</p>}
        <span className="text-[11px] text-on-surface-variant flex items-center gap-1.5 mt-2.5 font-bold uppercase tracking-wider">
      ...
      {messages?.map((message, idx) => {
        ...
        return (
          <React.Fragment key={message._id}>
            {showDateSeparator && (
              <div className="flex justify-center my-5 animate-in fade-in">
                <span className="px-4 py-1.5 bg-surface-container border border-outline-variant/30 text-on-surface-variant text-[11px] font-bold uppercase tracking-widest rounded-full shadow-sm backdrop-blur-md">
                  {dateSeparatorText}
                </span>
              </div>
            )}
            <div className={`flex items-start gap-2 ${isMe ? "flex-row-reverse" : "flex-row"} group/container`}>
              ...
              {message.deletedForEveryone ? (
                ...
                <div className={`relative group/message ${message.linkPreview ? 'max-w-[85%] sm:max-w-sm' : 'max-w-[85%] sm:max-w-[70%]'} ${message.messageType === 'image' ? 'p-1.5' : 'px-4 py-2.5'} rounded-2xl flex flex-col shadow-sm transition-all ${isMe ? "bg-primary text-on-primary rounded-tr-sm" : "bg-surface-container-highest text-on-surface rounded-tl-sm border border-outline-variant/30"}`}>
                  ...
                  {message.messageType === 'document' && message.document && (
                    <div onClick={(e) => handleDownload(e, message._id, message.document!.originalName, message.document!.url)} className={`flex items-center gap-3 p-3 mt-1 mb-1 rounded-xl transition-colors shadow-sm bg-surface-container hover:bg-surface-container-high border border-outline-variant/30 cursor-pointer`}>
                      <div className="w-10 h-10 rounded-lg bg-primary/10 flex items-center justify-center shrink-0">
                        <FileText size={20} className="text-primary" />
                      </div>
                      <div className="flex flex-col flex-1 min-w-0 pr-2">
                        <span className="text-sm font-semibold truncate text-on-surface">{message.document.originalName}</span>
                        <span className="text-xs text-on-surface-variant font-medium">{(message.document.size / 1024 / 1024).toFixed(2)} MB • {message.document.format}</span>
                      </div>
                      <div className="w-8 h-8 rounded-full bg-surface hover:bg-surface-container-highest flex items-center justify-center text-on-surface-variant hover:text-primary transition-colors shrink-0 shadow-sm border border-outline-variant/20">
                        <Download size={14} />
                      </div>
                    </div>
                  )}
                  {message.text && (
                    <div className={`text-[15px] leading-relaxed break-words whitespace-pre-wrap ${message.messageType === 'image' ? 'px-2 pb-0 pt-1.5' : ''}`}>
                  ...
                  <div className={`flex items-center justify-end gap-1 mt-0.5 text-[11px] font-medium ${message.messageType === 'image' && !message.text ? 'absolute bottom-3 right-3 px-2 py-1 bg-black/40 backdrop-blur-sm rounded-full text-white z-10' : (isMe ? 'text-on-primary/80' : 'text-on-surface-variant/80')}`}>
                    {message.isEdited && <span className="italic mr-1 opacity-80 text-[10px]">Edited</span>}
                    <span>{new Date(message.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                    {isMe && (
                            ...
                            <div className="py-1">
                              {!isMe && message.messageType === 'image' && message.image && (
                                <button
                                  ...
                                  className="w-full text-left px-4 py-2.5 text-sm hover:bg-surface-container text-on-surface flex items-center gap-2.5 transition-colors cursor-pointer border-b border-outline-variant/10"
                                >
