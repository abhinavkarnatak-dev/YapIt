...
export const getAllChats = TryCatch(async (req: AuthenticatedRequest, res) => {
    ...
    const chatWithUserData = await Promise.all(
        chats.map(async (chat) => {
            ...
            try {
                const { data } = await axios.get<any>(`http://user-service:8080/api/v1/user/${otherUserId}`);
...
export const sendMessage = TryCatch(async (req: AuthenticatedRequest, res) => {
    ...
    if (text) {
        ...
        if (urls && urls.length > 0) {
            ...
            try {
                if (isYT) {
                    const res = await axios.get<any>(`https://noembed.com/embed?url=${encodeURIComponent(firstUrl)}`);
                    ...
                    const res = await axios.get<any>(`https://api.microlink.io?url=${encodeURIComponent(firstUrl)}`);
...
export const markMessagesAsSeen = TryCatch(async (req: AuthenticatedRequest, res) => {
    const userId = req.user?._id;
    const { chatId } = req.params;
...
export const downloadDocument = TryCatch(async (req: AuthenticatedRequest, res) => {
    const userId = req.user?._id;
    const { messageId } = req.params;
...
export const editMessage = TryCatch(async (req: AuthenticatedRequest, res) => {
    ...
    if (urls && urls.length > 0) {
        ...
        try {
            if (isYT) {
                const resReq = await axios.get<any>(`https://noembed.com/embed?url=${encodeURIComponent(firstUrl)}`);
                ...
                const resReq = await axios.get<any>(`https://api.microlink.io?url=${encodeURIComponent(firstUrl)}`);
